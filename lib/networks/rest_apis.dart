...
Future<SubscriptionResponse> saveSubscriptionDetails(Map request) async {
  return SubscriptionResponse.fromJson(await handleResponse(
      await buildHttpResponse('subscription-transactions',
          request: request, method: HttpMethodType.POST)));
}
...
Future<void> addServiceMultiPart(
  ...
  await sendMultiPartRequest(multiPartRequest, onSuccess: (temp) async {
    ...
    appStore.setLoading(false);
    print('erorr occured while add services ----------> $s');
    toast(e.toString());
  });
}
//endregion
...
Future<DailyReportResponse> getDailyReportsAPI({
  required String bookingId,
}) async {
  try {
    final response = await handleResponse(
      await buildHttpResponse(
        'daily-wash-reports/$bookingId',
        method: HttpMethodType.GET,
      ),
    );

    return DailyReportResponse.fromJson(response);
  } catch (e) {
    throw e;
  }
}
...
Future<void> submitDailyReportAPI({
  required String bookingId,
  ...
  final uri = Uri.parse('${BASE_URL}daily-wash-reports/submit'); // ✅ fixed

  final request = http.MultipartRequest('POST', uri)
    ..headers.addAll(await buildAuthHeader())
    ..fields['booking_id'] = bookingId
    ..fields['service_id'] = serviceId
    ..fields['service_plan_id'] = servicePlanId
    ..fields['date'] = date;
  ...
  log('Submit Report Response: $responseBody');
